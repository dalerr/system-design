...
**Определение:**
Архитектура, где серверы не хранят состояние между запросами. Каждый запрос независим и содержит всю необходимую информацию.

**Характеристики:**
- Серверы не хранят состояние между запросами
- Каждый запрос независим
- Состояние хранится во внешних системах (БД, кеш, сессионное хранилище)
- Легко масштабируется горизонтально

**Примеры stateless:**
- REST API сервисы
- Микросервисы без локального состояния
- Веб-серверы с сессиями в Redis
- Serverless функции

**Преимущества stateless:**
- ✅ Легкое горизонтальное масштабирование — можно добавлять/удалять серверы без проблем
- ✅ Нет проблем с синхронизацией состояния
- ✅ Простой failover — любой сервер может обработать запрос
- ✅ Равномерное распределение нагрузки
- ✅ Проще деплой и обновление

**Недостатки stateless:**
- ❌ Нужно хранить состояние во внешних системах
- ❌ Дополнительная задержка при доступе к состоянию
- ❌ Нужна инфраструктура для хранения состояния
...
**Определение:**
Архитектура, где серверы хранят состояние между запросами. Состояние привязано к конкретному серверу.

**Характеристики:**
- Серверы хранят состояние между запросами
- Состояние привязано к конкретному серверу
- Сложнее масштабировать — нужно учитывать, где хранится состояние

**Примеры stateful:**
- WebSocket соединения
- Игровые серверы с состоянием игры
- Файловые серверы с локальными файлами
- Серверы с in-memory кешем без внешнего хранилища
- Stateful микросервисы (редко)

**Проблемы stateful:**
- ❌ Сложное масштабирование — нужно переносить состояние
- ❌ Sticky sessions требуются для привязки пользователя к серверу
- ❌ При падении сервера теряется состояние
- ❌ Неравномерное распределение нагрузки
- ❌ Сложнее деплой и обновление

**Стратегии для stateful систем:**

**1. Sticky sessions (session affinity):**
- Load balancer направляет запросы одного пользователя на один сервер
- Простота реализации
- Проблема: неравномерное распределение нагрузки

**2. External state storage:**
- Хранить состояние в Redis, БД, или другом внешнем хранилище
- Превращает stateful в stateless
- Рекомендуемый подход

**3. State replication:**
- Реплицировать состояние между серверами
- Сложность синхронизации
- Высокая задержка

**4. Миграция к stateless:**
- По возможности выносить состояние во внешние системы
...
|----------------|-----------|----------|
| **Масштабирование** | Легкое | Сложное |
| **Failover** | Простой | Сложный |
| **Распределение нагрузки** | Равномерное | Неравномерное |
| **Деплой** | Простой | Сложный |
| **Хранение состояния** | Внешнее | Локальное |
| **Latency** | Может быть выше | Может быть ниже |
| **Использование** | Рекомендуется | Только при необходимости |

---

### Рекомендации

**Предпочитайте stateless архитектуру:**
- Для легкого масштабирования
- Для высокой доступности
- Для упрощения операций

**Используйте stateful только когда необходимо:**
- WebSocket соединения
- Real-time игры
- Специфичные требования к производительности
- Когда нельзя использовать внешнее хранилище

**Миграция от stateful к stateless:**
1. Вынести состояние во внешнее хранилище (Redis, БД)
2. Использовать stateless серверы
3. Хранить сессии в Redis
4. Использовать stateless микросервисы
