...
2. [Общие вопросы по системному дизайну](#2-общие-вопросы-по-системному-дизайну-10-минут)
   - [Функциональные и нефункциональные требования](#1-функциональные-и-нефункциональные-требования)
   - [Масштабирование (вертикальное/горизонтальное, stateless/stateful)](#2-масштабирование)
   - [Стратегии отказоустойчивости](#3-стратегии-отказоустойчивости)
   - [CAP-теорема](#4-cap-теорема)
   - [Event-driven архитектуры](#5-event-driven-архитектуры)
   - [ACID vs BASE](#6-acid-vs-base)
   - [Микросервисы vs модульный монолит](#7-микросервисы-vs-модульный-монолит)
   - [Проектирование API](#8-проектирование-api)
   - [Мониторинг и наблюдаемость](#9-мониторинг-и-наблюдаемость)
   - [Глубокое погружение в масштабирование](#10-глубокое-погружение-в-масштабирование)
   - [Навык принятия решений](#11-навык-принятия-решений)
...
2. **Масштабирование** (вертикальное / горизонтальное, stateless / stateful)
...
10. **Глубокое погружение в масштабирование**
11. **Навык принятия решений**
...
- **Stateless** — серверы не хранят состояние, легко масштабируется горизонтально
- **Stateful** — серверы хранят состояние, сложнее масштабировать
- Предпочитайте stateless архитектуру для легкого масштабирования

**Подробнее:** см. документ [scaling.md](./scaling.md)

**Ожидаемое поведение:**
- Понимает разницу между stateless и stateful
- Понимает разницу между вертикальным и горизонтальным масштабированием
- Знает, когда что применять
...
**Типы API:**
- **REST** — стандарт для веб-сервисов, использует HTTP методы
- **GraphQL** — язык запросов, клиент определяет структуру ответа
- **gRPC** — высокопроизводительный RPC для внутренних сервисов

**Ключевые концепции:**
- Версионирование API (URL, header, query параметр)
- Идемпотентность — многократное выполнение дает тот же эффект
- API Gateway — единая точка входа с общими функциями
- Документация (OpenAPI/Swagger)

**Ожидаемое поведение:**
- Понимает разницу между REST, GraphQL и gRPC
- Может выбрать подходящий тип API для конкретного случая
- Понимает важность версионирования и идемпотентности
- Учитывает безопасность и производительность

**Подробнее:** см. документ [api-design.md](./api-design.md)

#### 9. Мониторинг и наблюдаемость

**Три столпа наблюдаемости:**
- **Метрики** — числовые значения для алертинга и визуализации
- **Логи** — записи событий для дебага и аудита
- **Трейсинг** — отслеживание запросов через распределенную систему

**Типы метрик:**
- Инфраструктурные (CPU, memory, disk, network)
- Приложения (RPS, latency, error rate)
- Бизнес-метрики (пользователи, транзакции, конверсия)

**Алертинг:**
- Настройка правильных порогов
- Runbooks для реагирования
- Приоритизация алертов

**Ожидаемое поведение:**
- Понимает важность мониторинга и наблюдаемости
- Знает основные метрики для мониторинга
- Понимает разницу между метриками, логами и трейсами
- Может предложить стратегию мониторинга для системы

**Подробнее:** см. документ [monitoring.md](./monitoring.md)

#### 10. Глубокое погружение в масштабирование

**Ключевые области:**
- **Capacity Planning** — планирование мощностей для будущей нагрузки
- **Bottleneck Analysis** — поиск узких мест (CPU, memory, I/O, database-bound)
- **Performance Testing** — нагрузочное тестирование (load, stress, spike, endurance)
- **Оптимизация** — на уровне приложения, БД, инфраструктуры

**Ожидаемое поведение:**
- Может провести capacity planning
- Умеет находить узкие места в системе
- Знает методы оптимизации на разных уровнях
- Понимает trade-offs между оптимизацией и сложностью

**Подробнее:** см. документ [deep-scaling.md](./deep-scaling.md)

#### 11. Навык принятия решений

**Процесс принятия решений:**
1. Анализ требований (функциональных, нефункциональных, ограничений)
2. Исследование вариантов
3. Оценка trade-offs (консистентность vs производительность, простота vs гибкость)
4. Выбор решения с обоснованием

**Критерии выбора:**
- Технические (производительность, масштабируемость, надежность)
- Нетехнические (стоимость, экспертиза команды, зрелость технологии)

**Методы:**
- Приоритизация (MoSCoW: Must, Should, Could, Won't)
- Итеративный подход (начинать с простого, улучшать по мере необходимости)
- Документирование (ADR — Architecture Decision Records)

**Ожидаемое поведение:**
- Анализирует требования перед принятием решения
- Рассматривает несколько вариантов
- Оценивает trade-offs
- Обосновывает выбор
- Документирует решения

**Подробнее:** см. документ [decision-making.md](./decision-making.md)
...
- ✅ Раздел по проектированию API (REST, GraphQL, gRPC)
- ✅ Раздел по мониторингу и наблюдаемости (метрики, логи, трейсинг)
- ✅ Раздел по глубокому погружению в масштабирование (capacity planning, bottleneck analysis)
- ✅ Раздел по навыку принятия решений (trade-offs, критерии выбора)
- ✅ Расширенный раздел по масштабированию (stateless vs stateful)
